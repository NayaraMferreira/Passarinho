...
let alturaCano = 500; // Altura dos canos em pixels
...
let velocidadeY = -5  ; // Velocidade de salto do pássaro
let gravidade = 0.6; // Gravidade aplicada ao pássaro
...
function atualizar() {
    ...
    if (jogoEncerrado) {
        ...
        contexto.fillText("FIM DE JOGO", 80, 250);
        contexto.fillText(`Pontuação: ${Math.floor(pontuacao)}`, 120, 300);
        contexto.fillText(`Dinheiro: $${dinheiroUsuario.toFixed(2)}`, 120, 350);
        return;
    }
    ...
    for (let i = 0; i < arrayCanos.length; i++) {
        ...
        if (!cano.passou && passaro.x > cano.x + cano.largura) {
            pontuacao += 1; // Incrementa a pontuação por meio ponto
            cano.passou = true;
            ganharDinheiro(5); // Marca que o pássaro já passou por esse cano
        }
    ...
    // Pontuação
    pontuacao += 0.1 + dinheiroGanho * 0.01; // Ajuste a lógica de pontuação

    // Ajusta a velocidade dos canos com base no dinheiro do usuário
    velocidadeX = -2 - dinheiroUsuario * 0.01; // Quanto mais dinheiro, mais rápido os canos se movem

    // Desenha o placar incluindo a quantidade fictícia de dinheiro ganho
    contexto.fillStyle = "white";
    contexto.font = "20px sans-serif";
    contexto.fillText(`Pontuação: ${Math.floor(pontuacao)}`, 10, 20);
    contexto.fillText(`Dinheiro: $${dinheiroUsuario.toFixed(2)}`, 10, 40);
}


function ganharDinheiro(quantidade) {
    dinheiroGanho += quantidade;
    dinheiroUsuario += quantidade;
}
